    ...
    def hash_board(self, board: chess.BaseBoard) -> int:
        ...
        for pivot, squares in enumerate(board.occupied_co):
            for square in chess.scan_reversed(squares):
                piece_index = (
                    typing.cast(chess.PieceType, board.piece_type_at(square)) - 1
                ) * 2 + pivot
...
class MemoryMappedReader:
    ...
    def __init__(self, filename: PathLike) -> None:
        self.fd = os.open(
            filename,
            os.O_RDONLY | os.O_BINARY if hasattr(os, "O_BINARY") else os.O_RDONLY,
        )

        try:
            self.mmap: Union[mmap.mmap, _EmptyMmap] = mmap.mmap(
                self.fd, 0, access=mmap.ACCESS_READ
            )
        ...
        if self.mmap.size() % ENTRY_STRUCT.size != 0:
            raise IOError(
                f"invalid file size: ensure {filename!r} is a valid polyglot opening book"
            )
    ...
    def bisect_key_left(self, key: int) -> int:
        ...
        while lo < hi:
            mid = (lo + hi) // 2
            mid_key, _, _, _ = ENTRY_STRUCT.unpack_from(
                self.mmap, mid * ENTRY_STRUCT.size
            )
            if mid_key < key:
                lo = mid + 1
            else:
    ...
    def __contains__(self, entry: Entry) -> bool:
        return any(
            current == entry
            for current in self.find_all(entry.key, minimum_weight=entry.weight)
        )
    ...
    def find_all(
        ...
        *,
        ...
        while i < size:
            ...
            if context:
                move = context._from_chess960(
                    context.chess960,
                    entry.move.from_square,
                    entry.move.to_square,
                    entry.move.promotion,
                    entry.move.drop,
                )
                entry = Entry(
                    entry.key, entry.raw_move, entry.weight, entry.learn, move
                )
    ...
    def find(
        ...
        *minimum_weight* ``0`` to select all entries.
        ...
        try:
            return max(
                self.find_all(
                    board, minimum_weight=minimum_weight, exclude_moves=exclude_moves
                ),
    ...
    def weighted_choice(
        ...
        *,
        ...
        """
        total_weights = sum(
            entry.weight for entry in self.find_all(board, exclude_moves=exclude_moves)
        )
