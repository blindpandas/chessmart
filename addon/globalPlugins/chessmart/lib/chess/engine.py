...
class EventLoopPolicy(asyncio.AbstractEventLoopPolicy):
    ...
    def get_event_loop(self) -> asyncio.AbstractEventLoop:
        ...
        if self._local.loop is None:
            raise RuntimeError(
                f"no current event loop in thread {threading.current_thread().name!r}"
            )
    ...
    def parse(self, value: ConfigValue) -> ConfigValue:
        if self.type == "check":
            ...
            try:
                ...
                raise EngineError(
                    f"expected integer for spin option {self.name!r}, got: {value!r}"
                )
            if self.min is not None and value < self.min:
                raise EngineError(
                    f"expected value for option {self.name!r} to be at least {self.min}, got: {value}"
                )
            if self.max is not None and self.max < value:
                raise EngineError(
                    f"expected value for option {self.name!r} to be at most {self.max}, got: {value}"
                )
            return value
            ...
            if value not in (self.var or []):
                raise EngineError(
                    "invalid value for combo option {!r}, got: {} (available: {})".format(
            ...
            if "\n" in value or "\r" in value:
                raise EngineError(
                    f"invalid line-break in string option {self.name!r}: {value!r}"
                )
            return value
        else:
...
class PovScore:
    ...
    def __repr__(self) -> str:
        return "PovScore({!r}, {})".format(
            self.relative, "WHITE" if self.turn else "BLACK"
        )
...
class Mate(Score):
    ...
    def wdl(self, *, model: _WdlModel = "sf", ply: int = 30) -> Wdl:
        if model == "lichess":
            ...
            return (
                Wdl(wins, 0, 1000 - wins)
                if self.moves > 0
                else Wdl(1000 - wins, 0, wins)
            )
        else:
        ...
        while b"\n" in self.stdin_buffer:
            ...
            if line.startswith("ping ") and self.expected_pings:
                ...
                self.protocol.pipe_data_received(
                    1, line.replace("ping ", "pong ").encode("utf-8") + b"\n"
                )
            else:
...
class Protocol(asyncio.SubprocessProtocol, metaclass=abc.ABCMeta):
    ...
    async def communicate(
        self: ProtocolT,
        command_factory: Callable[[ProtocolT], BaseCommand[ProtocolT, T]],
    ) -> T:
        command = command_factory(self)

        if self.returncode.done():
            raise EngineTerminatedError(
                f"engine process dead (exit code: {self.returncode.result()})"
            )
    ...
    async def popen(
        ...
        *,
        setpgrp: bool = False,
        **popen_args: Any,
    ) -> Tuple[asyncio.SubprocessTransport, ProtocolT]:
...
class BaseCommand(Generic[ProtocolT, T]):
    ...
    def _engine_terminated(self, engine: ProtocolT, code: int) -> None:
        exc = EngineTerminatedError(
            f"engine process died unexpectedly (exit code: {code})"
        )
    ...
    def set_finished(self) -> None:
        ...
        if not self.result.done():
            self.result.set_exception(
                EngineError(
                    f"engine command finished before returning result: {self!r}"
                )
            )
        self.finished.set_result(None)
    ...
    def __repr__(self) -> str:
        return "<{} at {:#x} (state={}, result={}, finished={}>".format(
            type(self).__name__, id(self), self.state, self.result, self.finished
        )
...
class UciProtocol(Protocol):
    ...
    async def initialize(self) -> None:
        class UciInitializeCommand(BaseCommand[UciProtocol, None]):
            ...
            def _option(self, engine: UciProtocol, arg: str) -> None:
                ...
                without_default = Option(
                    " ".join(name), " ".join(type), None, min, max, var
                )
                option = Option(
                    without_default.name,
                    without_default.type,
                    without_default.parse(" ".join(default)),
                    min,
                    max,
                    var,
                )
                ...
                if (
                    ...
                    and not option.is_managed()
                    and option.name.lower() != "uci_analysemode"
                ):
    ...
    def _changed_options(self, options: ConfigMapping) -> bool:
        return any(
            value is None or value != self.config.get(name)
    ...
    def _setoption(self, name: str, value: ConfigValue) -> None:
        try:
            ...
            raise EngineError(
                "engine does not support option {} (available options: {})".format(
    ...
    def _configure(self, options: ConfigMapping) -> None:
        for name, value in collections.ChainMap(options, self.target_config).items():
            if name.lower() in MANAGED_OPTIONS:
                raise EngineError(
                    "cannot set {} which is automatically managed".format(name)
                )
    ...
    async def play(
        ...
        *,
        ...
        self.last_move = (
            board.move_stack[-1]
            if (same_game and ponder and board.move_stack)
        ...
        class UciPlayCommand(BaseCommand[UciProtocol, PlayResult]):
            def __init__(self, engine: UciProtocol):
                ...
                engine.may_ponderhit = (
                    board
                    if ponder
                    and not engine.first_game
                    and game == engine.game
                    and not engine._changed_options(options)
                    else None
                )
            ...
            def _bestmove(self, engine: UciProtocol, arg: str) -> None:
                if self.pondering:
                    ...
                    self.result.set_result(
                        PlayResult(best.move, best.ponder, self.info)
                    )
            ...
            def cancel(self, engine: UciProtocol) -> None:
                if (
                    engine.may_ponderhit
                    and self.pondering
                    and engine.may_ponderhit.move_stack == self.pondering.move_stack
                    and engine.may_ponderhit == self.pondering
                ):
...
class UciOptionMap(MutableMapping[str, T]):
    ...
    def __init__(
        self, data: Optional[Union[Iterable[Tuple[str, T]]]] = None, **kwargs: T
    ) -> None:
...
class XBoardProtocol(Protocol):
    ...
    async def initialize(self) -> None:
        class XBoardInitializeCommand(BaseCommand[XBoardProtocol, None]):
            ...
            def start(self, engine: XBoardProtocol) -> None:
                ...
                self.timeout_handle = engine.loop.call_later(
                    2.0, lambda: self.timeout(engine)
                )
            ...
            def end(self, engine: XBoardProtocol) -> None:
                ...
                if engine.features.get("memory", 0):
                    engine.options["memory"] = Option(
                        "memory", "spin", 16, 1, None, None
                    )
    ...
    def _variant(self, variant: Optional[str]) -> None:
        ...
        if not variant or variant not in variants:
            raise EngineError(
                "unsupported xboard variant: {} (available: {})".format(
    ...
    def _new(self, board: chess.Board, game: object, options: ConfigMapping) -> None:
        ...
        for move in board.move_stack[common_stack_len:]:
            if not move:
                LOGGER.warning(
                    "Null move (in %s) may not be supported by all XBoard engines",
                    self.board.fen(),
                )
        ...
        class XBoardPlayCommand(BaseCommand[XBoardProtocol, PlayResult]):
            def start(self, engine: XBoardProtocol) -> None:
                ...
                if limit.remaining_moves or clock is not None or increment is not None:
                    ...
                    engine.send_line(
                        f"level {limit.remaining_moves or 0} {base_mins}:{base_secs:02d} {increment or 0}"
                    )
                ...
                if limit.white_clock is not None:
                    engine.send_line(
                        "{} {}".format(
                            "time" if board.turn else "otim",
                            max(1, int(limit.white_clock * 100)),
                        )
                    )
                if limit.black_clock is not None:
                    engine.send_line(
                        "{} {}".format(
                            "otim" if board.turn else "time",
                            max(1, int(limit.black_clock * 100)),
                        )
                    )
    ...
    async def analysis(
        ...
        *,
        ...
        if limit is not None and (
            limit.white_clock is not None or limit.black_clock is not None
        ):
        ...
        class XBoardAnalysisCommand(BaseCommand[XBoardProtocol, AnalysisResult]):
            def start(self, engine: XBoardProtocol) -> None:
                ...
                if limit is not None and limit.time is not None:
                    self.time_limit_handle: Optional[
                        asyncio.Handle
                    ] = engine.loop.call_later(limit.time, lambda: self.cancel(engine))
                else:
            ...
            def _post(self, engine: XBoardProtocol, line: str) -> None:
                ...
                if limit is not None:
                    if (
                        limit.time is not None
                        and post_info.get("time", 0) >= limit.time
                    ):
                        self.cancel(engine)
                    elif (
                        limit.nodes is not None
                        and post_info.get("nodes", 0) >= limit.nodes
                    ):
                        self.cancel(engine)
                    elif (
                        limit.depth is not None
                        and post_info.get("depth", 0) >= limit.depth
                    ):
                        self.cancel(engine)
...
def _parse_xboard_post(
    line: str, root_board: chess.Board, selector: Info = INFO_ALL
) -> InfoDict:
...
async def popen_uci(
    ...
    """
    transport, protocol = await UciProtocol.popen(
...
async def popen_xboard(
    command: Union[str, List[str]], *, setpgrp: bool = False, **popen_args: Any
) -> Tuple[asyncio.SubprocessTransport, XBoardProtocol]:
    """
    ...
    Returns a subprocess transport and engine protocol pair.
    """
    transport, protocol = await XBoardProtocol.popen(
        command, setpgrp=setpgrp, **popen_args
    )
    try:
        await protocol.initialize()
...
class SimpleEngine:
    ...
    *timeout* seconds longer than expected (unless *timeout* is ``None``).
    ...
    def communicate(
        self, command_factory: Callable[[Protocol], BaseCommand[Protocol, T]]
    ) -> T:
        with self._not_shut_down():
        ...
        *,
        ...
        with self._not_shut_down():
            coro = asyncio.wait_for(
                ...
                self.timeout,
            )  # Timeout until analysis is *started*
            future = asyncio.run_coroutine_threadsafe(coro, self.protocol.loop)
    ...
    def popen(
        ...
        **popen_args: Any,
        ...
        async def background(future: concurrent.futures.Future[SimpleEngine]) -> None:
            transport, protocol = await Protocol.popen(
                command, setpgrp=setpgrp, **popen_args
            )
            threading.current_thread().name = (
                f"{cls.__name__} (pid={transport.get_pid()})"
            )
        ...
        return run_in_background(
            background, name=f"{cls.__name__} (command={command!r})", debug=debug
        )
...
class SimpleAnalysisResult:
    ...
    def info(self) -> InfoDict:
        ...
        with self.simple_engine._not_shut_down():
            future = asyncio.run_coroutine_threadsafe(
                _get(), self.simple_engine.protocol.loop
            )
        return future.result()
    ...
    def wait(self) -> BestMove:
        with self.simple_engine._not_shut_down():
            future = asyncio.run_coroutine_threadsafe(
                self.inner.wait(), self.simple_engine.protocol.loop
            )
        return future.result()
    ...
    def empty(self) -> bool:
        ...
        with self.simple_engine._not_shut_down():
            future = asyncio.run_coroutine_threadsafe(
                _empty(), self.simple_engine.protocol.loop
            )
        return future.result()
    ...
    def get(self) -> InfoDict:
        with self.simple_engine._not_shut_down():
            future = asyncio.run_coroutine_threadsafe(
                self.inner.get(), self.simple_engine.protocol.loop
            )
        return future.result()
    ...
    def next(self) -> Optional[InfoDict]:
        with self.simple_engine._not_shut_down():
            future = asyncio.run_coroutine_threadsafe(
                self.inner.next(), self.simple_engine.protocol.loop
            )
        return future.result()
    ...
    def __next__(self) -> InfoDict:
        try:
            with self.simple_engine._not_shut_down():
                future = asyncio.run_coroutine_threadsafe(
                    self.inner.__anext__(), self.simple_engine.protocol.loop
                )
            return future.result()
