...
class SuicideBoard(chess.Board):
    ...
    def _attacked_for_king(
        self, path: chess.Bitboard, occupied: chess.Bitboard
    ) -> bool:
        return False
    ...
    def has_insufficient_material(self, color: chess.Color) -> bool:
        ...
        return (we_some_on_light and they_all_on_dark) or (
            we_some_on_dark and they_all_on_light
        )
...
class AtomicBoard(chess.Board):
    ...
    def has_insufficient_material(self, color: chess.Color) -> bool:
        ...
        if self.occupied_co[not color] & ~self.kings:
            ...
            if self.occupied == self.bishops | self.kings:
                if not (
                    self.bishops & self.occupied_co[chess.WHITE] & chess.BB_DARK_SQUARES
                ):
                    return not (
                        self.bishops
    ...
    def _kings_connected(self) -> bool:
        ...
        return any(
            chess.BB_KING_ATTACKS[sq] & black_kings
...
class RacingKingsBoard(chess.Board):
    ...
    def is_variant_end(self) -> bool:
        ...
        targets = (
            chess.BB_KING_ATTACKS[black_king]
            & chess.BB_RANK_8
            & ~self.occupied_co[chess.BLACK]
        )
        return all(
            self.attackers_mask(chess.WHITE, target)
    ...
    def is_variant_loss(self) -> bool:
        return (
            self.is_variant_end()
            and not self.kings & self.occupied_co[self.turn] & chess.BB_RANK_8
        )
    ...
    def is_variant_win(self) -> bool:
        ...
        return (
            self.is_variant_end()
            and bool(in_goal & self.occupied_co[self.turn])
            and not in_goal & self.occupied_co[not self.turn]
        )
...
class HordeBoard(chess.Board):
    ...
    def has_insufficient_material(self, color: chess.Color) -> bool:
        ...
        def has_bishop_pair(side: chess.Color) -> bool:
            return (
                (horde_lightb >= 1 and horde_darkb >= 1)
                if side == chess.WHITE
                else (pieces_lightb >= 1 and pieces_darkb >= 1)
            )
        ...
        if rooks >= 1 and horde_num >= 2:
            ...
            if not (
                ...
                and rooks == 1
                and bishops == 1
                and pieces_of_type_not(pieces_sameb_as(horde_bishop_co)) == 1
            ):
                return False
        ...
        if horde_num == 1:
            if pieces_num == 1:
                ...
                return promote_to_queen.has_insufficient_material(
                    chess.WHITE
                ) and promote_to_knight.has_insufficient_material(chess.WHITE)
            elif rooks == 1:
                ...
                return not (
                    pieces_pawns >= 2
                    or (pieces_rooks >= 1 and pieces_pawns >= 1)
                    or (pieces_rooks >= 1 and pieces_knights >= 1)
                    or (pieces_pawns >= 1 and pieces_knights >= 1)
                )
            elif bishops == 1:
                ...
                return not (
                    ...
                    or (pieces_oppositeb_of(horde_bishop_co) >= 1 and pieces_pawns >= 1)
                    or pieces_pawns >= 2
                )
            elif knights == 1:
                ...
                return not (
                    ...
                    and (
                        ...
                        or pieces_pawns >= 2
                        or (pieces_rooks >= 1 and pieces_knights >= 1)
                        or (pieces_rooks >= 1 and pieces_bishops >= 1)
                        or (pieces_knights >= 1 and pieces_bishops >= 1)
                        or (pieces_rooks >= 1 and pieces_pawns >= 1)
            ...
            if pieces_num == 1:
                ...
                return not (
                    ...
                    pieces_pawns >= 1
                    or pieces_oppositeb_of(horde_bishop_co) >= 1
                    or
                ...
                return not (
                    ...
                    (pieces_pawns >= 1 and pieces_oppositeb_of(horde_bishop_co) >= 1)
                    or (pieces_pawns >= 1 and pieces_knights >= 1)
                    or (
                        pieces_oppositeb_of(horde_bishop_co) >= 1
                        and pieces_knights >= 1
                    )
                    or (pieces_oppositeb_of(horde_bishop_co) >= 2)
                    or pieces_knights >= 2
                    or pieces_pawns >= 2
            ...
            if (
                (knights == 2 and bishops == 1)
                or knights == 3
                or has_bishop_pair(chess.WHITE)
            ):
                return False
            else:
...
class _ThreeCheckBoardState(
    Generic[ThreeCheckBoardT], chess._BoardState[ThreeCheckBoardT]
):
...
class ThreeCheckBoard(chess.Board):
    ...
    def epd(
        self,
        shredder: bool = False,
        en_passant: chess._EnPassantSpec = "legal",
    ...
    def is_variant_draw(self) -> bool:
        return (
            self.remaining_checks[chess.WHITE] <= 0
            and self.remaining_checks[chess.BLACK] <= 0
        )

    def is_variant_loss(self) -> bool:
        return (
            self.remaining_checks[not self.turn] <= 0 < self.remaining_checks[self.turn]
        )

    def is_variant_win(self) -> bool:
        return (
            self.remaining_checks[self.turn] <= 0 < self.remaining_checks[not self.turn]
        )
    ...
    def copy(
        self: ThreeCheckBoardT, stack: Union[bool, int] = True
    ) -> ThreeCheckBoardT:
...
class _CrazyhouseBoardState(
    Generic[CrazyhouseBoardT], chess._BoardState[CrazyhouseBoardT]
):
...
class CrazyhouseBoard(chess.Board):
    ...
    def generate_pseudo_legal_drops(
        ...
        for to_square in chess.scan_forward(to_mask & ~self.occupied):
            for pt, count in self.pockets[self.turn].pieces.items():
                if count and (
                    pt != chess.PAWN
                    or not chess.BB_BACKRANKS & chess.BB_SQUARES[to_square]
                ):
    ...
    def generate_legal_drops(
        ...
        return self.generate_pseudo_legal_drops(
            to_mask=self.legal_drop_squares_mask() & to_mask
        )
    ...
    def has_insufficient_material(self, color: chess.Color) -> bool:
        ...
        return (
            chess.popcount(self.occupied) + sum(len(pocket) for pocket in self.pockets)
            <= 3
    ...
    def copy(
        self: CrazyhouseBoardT, stack: Union[bool, int] = True
    ) -> CrazyhouseBoardT:
        board = super().copy(stack=stack)
