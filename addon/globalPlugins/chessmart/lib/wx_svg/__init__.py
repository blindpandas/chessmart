...
class SVGimage(SVGimageBase):
    ...
    def ConvertToBitmap(
        self, tx=0.0, ty=0.0, scale=1.0, width=-1, height=-1, stride=-1
    ):
        """
    ...
    def RenderToGC(self, ctx, scale=None, size=None):
        ...
        try:
            ...
            for shape in self.shapes:
                ...
                rule = {
                    SVG_FILLRULE_NONZERO: wx.WINDING_RULE,
                    SVG_FILLRULE_EVENODD: wx.ODDEVEN_RULE,
                }.get(shape.fillRule, 0)
    ...
    def _makeGradientStops(self, gradient):
        ...
        gcstops = wx.GraphicsGradientStops(
            wx.Colour(*first.color_rgba), wx.Colour(*last.color_rgba)
        )
        for stop in stops:
    ...
    def _makePen(self, ctx, shape):
        ...
        join = {
            SVG_JOIN_MITER: wx.JOIN_MITER,
            SVG_JOIN_ROUND: wx.JOIN_ROUND,
            SVG_JOIN_BEVEL: wx.JOIN_BEVEL,
        }.get(shape.strokeLineJoin, 0)
        cap = {
            SVG_CAP_BUTT: wx.CAP_BUTT,
            SVG_CAP_ROUND: wx.CAP_ROUND,
            SVG_CAP_SQUARE: wx.CAP_PROJECTING,
        }.get(shape.strokeLineCap, 0)
        # TODO: handle dashes
